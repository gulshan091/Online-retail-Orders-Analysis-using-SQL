...
# Question 2 :  Write a query to display the following information for the products, 
...
WHEN p.PRODUCT_PRICE > 200000 THEN p.PRODUCT_PRICE 
*0.8 
WHEN p.PRODUCT_PRICE > 100000 THEN p.PRODUCT_PRICE 
*0.85 
...
# Question 3 : Write a query to display Product_class_code, Product_class_description, 
# Count of Product type in each productclass, Inventory Value (p.product_quantity_avail*p.product_price).
# Information should be displayed for only those product_class_code which have more than 1,00,000. 
...
from 
(SELECT oh.customer_id, concat(customer_fname,' ',customer_lname) as 
Full_name, oc.customer_email, 
oc.customer_phone, a.country from order_header oh left join online_customer 
oc on oh.customer_id = oc.customer_id 
left join address a on oc.address_id=a.address_id where oh.order_status = 
'Cancelled') s 
where customer_id not in (Select oh.customer_id from order_header oh where 
oh.order_status != 'Cancelled');

# Question 5 : Write a query to display Shipper name, City to which it is catering, 
# num of customer catered by the shipper in the city and number of consignments delivered 
# to that city for Shipper DHL [NOTE: TABLES to be used - SHIPPER,ONLINE_CUSTOMER, ADDRESSS, ORDER_HEARDER]
...
# Question 6 : Write a query to display product_id, product_desc, product_quantity_avail, 
...
# 'No Sales in past, give discount to reduce inventory', if inventory quantity is less than 10% of quantity sold,
# show 'Low inventory, need to add inventory', if inventory quantity is less than 50% of quantity sold, 
...
# c. Rest of the categories, if sales till date is Zero then show 'No Sales in past, 
# give discount to reduce inventory', if inventory quantity is less than 30% of quantity sold, show 'Low inventory, 
# need to add inventory', if inventory quantity is less than 70% of quantity sold, show 'Medium inventory,
# need to add some inventory', if inventory quantity is more or equal to 70% of quantity sold, 
# show 'Sufficient inventory' -- (USE SUB-QUERY) -- [NOTE: TABLES to be used - PRODUCT, PRODUCT_CLASS, ORDER_ITEMS]
...
CASE
WHEN (product_quantity_avail/S.tot_qty) <0.1 THEN "Low
inventory, need to add inventory"
WHEN (product_quantity_avail/S.tot_qty) <0.5 THEN "Medium
inventory, need to add some inventory"
WHEN S.tot_qty is NULL THEN "No Sales in past, give discount
to reduce inventory"
ELSE "Sufficient inventory" 
END
WHEN product_class_desc in ('Mobiles','Watches') THEN
CASE
WHEN (product_quantity_avail/S.tot_qty) <0.2 then "Low
inventory, need to add inventory"
WHEN (product_quantity_avail/S.tot_qty) <0.6 then "Medium
inventory, need to add some inventory"
WHEN S.tot_qty is null then "No Sales in past, give discount to
reduce inventory"
ELSE "Sufficient inventory"
END
ELSE
CASE
WHEN (product_quantity_avail/S.tot_qty) <0.3 then "Low
inventory, need to add inventory"
WHEN (product_quantity_avail/S.tot_qty) <0.7 then "Medium
inventory, need to add some inventory"
WHEN S.tot_qty is null then "No Sales in past, give discount to
reduce inventory"
ELSE "Sufficient inventory"
...
FROM (SELECT p.product_id, p.product_desc,
pc.product_class_desc,p.product_quantity_avail, SUM(oi.product_quantity)
AS tot_qty FROM Product p left join order_items oi on oi.product_id =
p.product_id left join product_class pc
on p.product_class_code = pc.product_class_code GROUP BY p.product_id,
p.product_desc) S;


# Question 7 : Write a query to display order_id and volume of the biggest order (in terms of volume) 
...
# Question 8 : Write a query to display customer id, customer full name, 
# total quantity and total value (quantity*price) shipped where mode of payment is Cash and
# customer last name starts with 'G' 
...
# Question 9: Write a query to display product_id, 
# product_desc and total quantity of products which are sold together with product id 201 and are not shipped to city
# Bangalore and New Delhi. Display the output in descending order with respect to the tot_qty. 
...
AND order_id IN (SELECT order_id FROM Order_Items WHERE product_id = 201)
...
# total quantity of products shipped for order ids which are even and shipped to address 
